...
# Added required imports for the tasks
...
def summarize_one(summ, text: str, max_length: int = 120, min_length: int = 30) -> str:
    ...
    result = summ(
        text, max_length=max_length, min_length=min_length, do_sample=False, num_beams=4
    )
...
def main() -> None:
    ...
    metrics_path = (
        _output_path().replace("predictions", "metrics").replace(".csv", ".json")
    )
